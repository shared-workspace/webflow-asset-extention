...
import { WebflowClient } from "webflow-api"; // Interact with the Webflow API
...
class App {
  ...
  data: Level<string, string>;
  oauth: AuthorizationCode<"client_id">;
  constructor(clientId: string,private clientSecret: string) {
  ...
  async install(code: string){
    const access = await this.oauth.getToken({
      code,
      redirect_uri: ""
    });
    this.storeToken(access.token.access_token as string);
   ...
   */
  async addWebhooks(triggerType: string, url: string, token: string) {
